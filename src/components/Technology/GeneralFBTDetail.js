...
const GeneralFBTDetail = () => {
  ...
  const singleData = generalFBTData.filter((Post) => Post.id === idNum);
  const relatedData = generalFBTData.filter((Post) => Post.id !== idNum);
  return (
    <div className="grid lg:grid-cols-3 lg:gap-6 w-full h-auto px-4 py-4 mt-16">
      <SinglePost title="General/FBT" singleData={singleData} />
