...
import SingleMovieC from "../SingleMovieC";

const WrestlingDetail = () => {
  //trending data
  ...
  const singleData = wrestlingData.filter((Post) => Post.id === idNum);
  const relatedData = wrestlingData.filter((Post) => Post.id !== idNum);
  return (
    <div className="grid lg:grid-cols-3 lg:gap-6 w-full h-auto px-4 py-4 mt-16">
      <SingleMovieC title="Wrestling" singleData={singleData} />
      <div className="w-full lg:col-span-1 py-8 md:pr-8 lg:pl-0 md:pl-8 mt-4">
        <CardModule title="Related Posts" data={relatedData} url="/wrestling" />
...
export default WrestlingDetail;
