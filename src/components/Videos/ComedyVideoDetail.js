...
const ComedyVideoDetail = () => {
  ...
  const singleData = comedyVideosData.filter((Post) => Post.id === idNum);
  const relatedData = comedyVideosData.filter((Post) => Post.id !== idNum);
  return (
    <div className="grid lg:grid-cols-3 lg:gap-6 w-full h-auto px-4 py-4 mt-16">
      <SingleVideoC title="Comedy Video" singleData={singleData} />
...
export default ComedyVideoDetail;
