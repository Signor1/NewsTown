...
const AfroMusicDetail = () => {
  ...
  const singleData = afroMusicData.filter((Post) => Post.id === idNum);
  const relatedData = afroMusicData.filter((Post) => Post.id !== idNum);
  return (
    <div className="grid lg:grid-cols-3 lg:gap-6 w-full h-auto px-4 py-4 mt-16">
      <SingleMedia title="Nigerian/Afro-Music" singleData={singleData} />
      <div className="w-full lg:col-span-1 py-8 md:pr-8 lg:pl-0 md:pl-8 mt-4">
        <CardModule title="Related Posts" data={relatedData} url="/afromusic" />
...
export default AfroMusicDetail;
